...
3. 选择以下任一方式配置密钥：
   - 点击`生成`创建随机密钥；
   - 点击`导入(.bin)`导入 32 字节密钥文件；
   - 点击`Hex 导入`粘贴 64 位十六进制密钥（可带 `0x` 前缀）。
4. 可选：点击`导出(.bin)`备份当前选中槽位密钥。
5. 可选：给当前激活槽位设置标签，便于后续辨识。

说明：
- 未配置密钥时，`嵌入`按钮会禁用。
- `检测`可以执行，但结果可能为未校验状态（仅供参考，不可用于归属/取证）。
- 仅 macOS：当前密钥按槽位保存为独立钥匙串条目（每槽位一个条目），首次访问多个已配置槽位时可能出现多次认证。
- 仅 macOS：在系统认证弹窗选择`始终允许`后，通常不会再重复询问同一“应用身份 + 条目”。
- 仅 macOS：若应用身份变化（如未签名/临时签名构建、重装、更新后签名变化），系统可能重新询问授权。这是系统安全机制，不是 AWMKit 功能故障。
