...
说明：这是 macOS Keychain 的授权机制行为，不是 AWMKit 功能故障。当前密钥按槽位存储（每槽位一个钥匙串条目），首次访问多个已配置槽位时可能出现多次认证。  

排查与处理：

1. 先确认是否在首次访问新的槽位密钥；首次访问多个槽位时，按条目多次询问是预期行为。
2. 在 Keychain Access 中检查对应条目的访问控制，确认当前应用是否已被授权。
3. 若在系统弹窗选择`始终允许`，通常不会再对同一“应用身份 + 条目”重复询问。
4. 若更新、重装或签名方式变化后又开始询问，通常是应用身份变化导致，需要重新授权。
5. 本地开发构建建议保持稳定签名/稳定应用身份后再观察认证行为。

补充：该行为仅适用于 macOS Keychain；Windows 不使用此授权模型。

## 8. 管道 I/O 兼容问题（`stdin/stdout`）
