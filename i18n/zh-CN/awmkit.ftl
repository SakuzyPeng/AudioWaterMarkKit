...
cli-key-slot-active = 当前为激活槽位
cli-key-slot-current_active = 当前激活槽位：{ $slot }
cli-key-imported-slot = [OK] 已导入密钥到槽位 { $slot }
cli-key-exported-slot = [OK] 已从槽位 { $slot } 导出密钥
cli-key-rotated-slot = [OK] 已轮换槽位 { $slot } 密钥
cli-key-delete-requires-yes = 删除密钥需要 --yes 确认
cli-key-delete-slot-has-evidence = 槽位 { $slot } 仍有 { $count } 条证据，请先处理冲突槽位（或使用 --force）
cli-key-deleted-slot = [OK] 已删除槽位 { $slot } 密钥
cli-key-slot-current = 激活槽位：{ $slot }
cli-key-slot-set = [OK] 已切换激活槽位到 { $slot }
cli-key-slot-label-set = [OK] 已设置槽位 { $slot } 标签：{ $label }
cli-key-slot-label-cleared = [OK] 已清除槽位 { $slot } 标签
