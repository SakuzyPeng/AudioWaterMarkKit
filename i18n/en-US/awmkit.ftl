...
cli-key-slot-active = This is the active slot
cli-key-slot-current_active = Current active slot: { $slot }
cli-key-imported-slot = [OK] key imported into slot { $slot }
cli-key-exported-slot = [OK] key exported from slot { $slot }
cli-key-rotated-slot = [OK] key rotated for slot { $slot }
cli-key-delete-requires-yes = deleting key requires --yes confirmation
cli-key-delete-slot-has-evidence = slot { $slot } still has { $count } evidence rows; resolve slot conflict first (or use --force)
cli-key-deleted-slot = [OK] deleted key in slot { $slot }
cli-key-slot-current = Active slot: { $slot }
cli-key-slot-set = [OK] active slot set to { $slot }
cli-key-slot-label-set = [OK] slot { $slot } label set: { $label }
cli-key-slot-label-cleared = [OK] slot { $slot } label cleared
