...
模拟真实世界中**恶意的、非标准的、极端劣质**的传播场景，测试水印的极限。

### 10.1 实验设计

| 实验 | 代号 | 模拟场景 | 组合公式 |
|------|------|----------|----------|
| Exp1 | Nightcore | 抖音神曲 | 变速1.15x + 升调2半音 + AAC 48k |
| Exp2 | Metal Mix | 恶劣搬运 | 混音1:1.5(重金属) + 粉红噪声 + MP3 64k |
| Exp3 | Telephone | 电话录音 | 45s裁剪 + 带通300-3400Hz + 8kHz + AAC 24k |
| Exp4 | Reverb Hell | 会议录音 | 强混响(1000ms) + 高通300Hz + 单声道 |

### 10.2 测试结果

| 实验 | 直接检测 | 逆向恢复 | 结论 |
|------|----------|----------|------|
| Exp1 Nightcore | ✗ 失败 | ✗ 失败 | **有效攻击** |
| Exp2 Metal Mix (真·重金属) | ✗ 失败 | - | **有效攻击** |
| Exp2 高响度混音 (对照组) | ✓ 通过 | - | 重金属频谱更具破坏性 |
| Exp3 Telephone | ✓ **通过** | - | 惊喜！带宽仅3kHz仍可检测 |
| Exp4 Reverb Hell | ✓ **通过** | - | 惊喜！强混响可检测 |
...
**惊喜发现**:
- 电话音质 (8kHz采样, 300-3400Hz带通) 仍可检测 - 水印在中低频段有足够能量
- 强混响 (1000ms延迟, 多次反射) 仍可检测 - 相关性检测鲁棒

**有效攻击手段**:
1. **Nightcore Attack**: 变速 + 变调 + 低码率的三重组合，即使逆向恢复也无法检测
   - 原因: AAC 48k 高频截断 + 双重时频失步 + 压缩损失
2. **真·重金属混音**: 真正的重金属音乐(而非单纯高响度)具有更强的频谱掩蔽能力
   - 原因: 重金属的宽频带高能量完全覆盖水印频段
